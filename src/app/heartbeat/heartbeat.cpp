...
#define DEFALUT_MQTT_IP_Teror "test.cn"
...
struct HeartbeatAppForeverData
{
    ...
    char liz_mqtt_subtopic[32]; // "ReMix-beat"
    char liz_mqtt_pubtopic[32]; // "ReMix-heart"
    char client_id[32];         // 用户设置的ID
    char subtopic[32];          // 订阅的Topic
    char connect_client_id[64]; // 连接是使用的client_id default "ReMix"+MAC+client_id
...
static void write_config(HeartbeatAppForeverData *cfg)
{
    ...
    if (cfg->role == 0)
    {
        snprintf(cfg->liz_mqtt_subtopic, 32, "ReMix-beat-%s", cfg->subtopic);
        snprintf(cfg->liz_mqtt_pubtopic, 32, "ReMix-heart-%s", cfg->client_id);
    }
    else
    {
        snprintf(cfg->liz_mqtt_subtopic, 32, "ReMix-heart-%s", cfg->subtopic);
        snprintf(cfg->liz_mqtt_pubtopic, 32, "ReMix-beat-%s", cfg->client_id);
    }
    memset(tmp, 0, 32);
...
static void read_config(HeartbeatAppForeverData *cfg)
{
    ...
    if (!strcmp(cfg->mqtt_server.toString().c_str(), DEFALUT_MQTT_IP) || !strcmp(cfg->mqtt_server.toString().c_str(), DEFALUT_MQTT_IP_CLIMBL) || !strcmp(cfg->mqtt_server.toString().c_str(), DEFALUT_MQTT_IP_Teror))
    {
        snprintf(cfg->connect_client_id, 64, "ReMix-%llu-%s", ESP.getEfuseMac(), cfg->client_id);
    }
    else
    {
...
APP_OBJ heartbeat_app = {HEARTBEAT_APP_NAME, &app_heartbeat, "Author WoodwindHu\nVersion 2.0.0\n",heartbeat_init, heartbeat_process, heartbeat_background_task,heartbeat_exit_callback, heartbeat_message_handle};
