...
void taskTwo(void *parameter)
{
    ...
    {
        // LVGL任务主函数
        ReMix_LVGL_OPERATE_LOCK(lv_task_handler();)
...
static int game_2048_init(AppController *sys)
{
    ...
    ReMix_LVGL_OPERATE_LOCK(showBoard(run_data->pBoard);)
    // 棋子出生动画
    ReMix_LVGL_OPERATE_LOCK(born(new1);)
    ReMix_LVGL_OPERATE_LOCK(born(new2);)
    // 防止进入游戏时，误触发了向上
                              ...
                              const ImuAction *act_info)
{
    ...
    if (TURN_RIGHT == act_info->active)
    {
        ...
        {
            ReMix_LVGL_OPERATE_LOCK(showAnim(run_data->moveRecord, 4);)
            delay(700);
            ReMix_LVGL_OPERATE_LOCK(showNewBorn(game.addRandom(), run_data->pBoard);)
        }
    }
    else if (TURN_LEFT == act_info->active)
    {
        ...
        if (game.comparePre() == 0)
        {
            ReMix_LVGL_OPERATE_LOCK(showAnim(run_data->moveRecord, 3);)
            delay(700);
            ReMix_LVGL_OPERATE_LOCK(showNewBorn(game.addRandom(), run_data->pBoard);)
        }
    }
    else if (UP == act_info->active)
    {
        ...
        if (game.comparePre() == 0)
        {
            ReMix_LVGL_OPERATE_LOCK(showAnim(run_data->moveRecord, 1);)
            delay(700);
            ReMix_LVGL_OPERATE_LOCK(showNewBorn(game.addRandom(), run_data->pBoard);)
        }
    }
    else if (DOWN == act_info->active)
    {
        ...
        if (game.comparePre() == 0)
        {
            ReMix_LVGL_OPERATE_LOCK(showAnim(run_data->moveRecord, 2);)
            delay(700);
            ReMix_LVGL_OPERATE_LOCK(showNewBorn(game.addRandom(), run_data->pBoard);)
        }
    }
