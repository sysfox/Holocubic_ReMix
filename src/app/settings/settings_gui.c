...
void display_settings(const char *cur_ver, const char *new_ver, lv_scr_load_anim_t anim_type)
{
    ...
    lv_label_set_text(title_label, "#00ff00 Hello\tin\tWorld#");
    ...
    if (strcmp(cur_ver, new_ver) != 0)
    {
        lv_label_set_text_fmt(new_ver_label, "Please update your ReMix to #ff0000 v %s#", new_ver);
        lv_obj_align(new_ver_label, LV_ALIGN_CENTER, 0, 60);
    }
    else
    {
        lv_label_set_text_fmt(new_ver_label, "ReMix #ff0000 v %s# is the latest version.", new_ver);
        lv_obj_align(new_ver_label, LV_ALIGN_CENTER, 0, 60);
    }
    lv_label_set_text(author_label, "@Teror Fox");
}
